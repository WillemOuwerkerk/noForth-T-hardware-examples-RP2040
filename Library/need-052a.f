...
07) COLOR-ON       ( -- )             Color markings on in CHAPTERS
08) COLOR-OFF      ( -- )             Color markings off in CHAPTERS
...
: COLOR     ( +n -- )
    ...
    base @ hex  swap 0 .r  base !   \ ASCII values of SGR parameters
    ch m emit  r> to hor ;          \ Close & correct HOR

: SGR       ( +n -- )
    sgr? if dup color then drop ;   \ Use graphic rendition?
...
: CHAPTERS  ( -- )      \ Show all chapters in current library
    ...
    while
        dup 3 - dup hardware) =     \ Separate library sectors
        swap pio) = or if cr space then
        2dup                        \ Save range
