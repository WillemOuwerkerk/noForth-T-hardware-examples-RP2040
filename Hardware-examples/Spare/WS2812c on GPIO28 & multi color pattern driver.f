...
\ of the LED string. In only 17 opcodes a flexible WS2812 LED driver,
...
0 0 {pio                        \ Use state machine-0 on PIO-0
    ...
    begin, 2>r
        ...
        y--? if,                \ Decrease Y by 1
            begin,  15 [] 0 side y--? until, \ Reset pulse
    2r@ again,
        then,
        begin,
...
: >LEDS     ( c +n -- )
    1 umax 400 umin  dup +to #pos >WS2812 ; \ Length is minimal 1 LED

: READY     ( -- )          0 0 >WS2812 ;   \ Ready signal
: ALL       ( c -- )        clr  #leds >leds  READY ; \ One color to leds

10 value #FIELD  \ Field length
03 value #DOT    \ Dot length
10 value #WAIT   \ Delay time
...
#leds >field    1 >dot      \ Basic size & color settings
red >fcolor    black >bcolor

: .MLINE    ( dot-pos -- dot-pos )
    dup >r                          \ Save DOT position
...
: MLINE     ( dot-pos -- )          \ Display one line with DOT
    ...
    clr  begin
        .mline
    #pos #leds u< 0= until          \ Whole LED string done?
    drop  READY ;
...
: MSHIFT    ( -- )                  \ Shift DOT until a key was pressed
    begin
        0 to #pos  #field 0 do  i mline  wait  key? if leave then  loop
    key? until  black all ;
...
: BOUNCE    ( -- )                  \ Bouncing DOT
    #dot  begin
        0 to #pos  #field #dot -  dup 0 ?do  i mline  wait  key? if leave then loop
