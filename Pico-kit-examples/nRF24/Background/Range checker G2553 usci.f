...
: CARRIER?      ( -- 0|1 )  \ Check for carrier
    ...
    2 /ms  9 read-nrf  write-mode ; \ Test for carrier on used frequency
...
: CHECK         ( -- )
    spi-on setup24L01  .status  read-mode \ Init.
    7E 0 do
        i 5 write-reg           \ Select channel number 0 to 7D
    ...
    loop  start24l01  read-mode ; \ Restore chosen channel


\ Show carriers on the channel number from the stack
: CARRIER   ( channel -- )      \ #CH = default channel
    spi-on setup24L01  .status  \ Initialise
    5 write-reg                 \ Set channel to be checked
    begin check) key? until     \ Check carrier on 'channel', show presence
    start24l01  read-mode ;     \ Restore chosen channel
...
: WAVE      ( channel power -- ) \ #CH = default channel
    ...
    5 write-reg  ce-high    \ Set channel
...
: PULSE     ( channel power pulselength -- ) \ #CH = default channel
    ...
    begin
        ...
        2 0 write-reg  15 /ms   \ Power up for constant carrier
        r@ 90 or  6 write-reg   \ Activate carrier, bitrate & power
        dup 5 write-reg  ce-high \ Set channel
